                    ...
                    public void onClick(View v) {
                        new ReportAsyncTask(new Handler(Looper.getMainLooper()) {
                            ...
                            public void handleMessage(Message msg) {
                                ...
                                if(msg.what==0) {
                                    Toast.makeText(mContext,"여행일을 신고했습니다.",Toast.LENGTH_LONG).show();
                                    mDayDialog.dismiss();
                                }
                            }
                        }).execute(mAccessToken,"days/"+feed.getId()+"/report");
                    }
                };
                String url = Session.WEP_DOMAIN+"days/"+feed.getId();
                View.OnClickListener edit = new ShareListner(mContext, url);
